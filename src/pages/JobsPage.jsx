...
const JobsPage = () => {
	...
	useEffect(() => {
		const fetchData = async () => {
			try {
				const response = await axios.get(`https://jobarena-backend.onrender.com/api/job/?lang=${language}`)
		...
		if (
			...
			const filteredData = jobData?.filter((item) => {
				return (
					...
					(!minStipend || item.CTC >= minStipend) &&
					(!keywords || item.aboutJob && item.aboutJob.includes(keywords))
				);
			});
					...
					<div className='flex flex-col md:flex-row items-end  md:items-start justify-between gap-x-4 mt-20 h-full sticky top-0'>
						...
						<div className='hidden md:grid grid-rows-[auto_1fr] w-fit h-min sticky top-0 md:text-sm'>
								...
								<form action='#' className='flex items-start justify-start flex-col gap-y-4 '>
									<div className='flex flex-col items-start justify-start gap-y-1 w-full mt-4 '>
										...
										<input
											...
											className='border w-full md:text-xs px-2 py-2 text-sm rounded-sm text-[--base-grey] hover:border-[--primary] focus-within:border-[--primary] outline-none tracking-wide '
											value={profile}
									...
									<div className='flex items-center justify-start gap-x-2 w-full '>
										...
										/>
										<label htmlFor='wfh' className='text-[--primary-text] md:text-sm font-medium '>
											{t("work from home")}
										</label>
									</div>
									<div className='flex items-center justify-start gap-x-2 w-full -mt-2 '>
										...
										/>
										<label htmlFor='part-time' className='text-[--primary-text] md:text-sm font-medium '>
											{t("part-time")}
										</label>
									</div>
									<div className='flex flex-col items-start justify-start gap-y-1 w-full '>
							...
							<div className='bg-white border border-[--grey] rounded-md px-4 py-8 mt-2  '>
								<p className='font-medium text-lg text-center text-[--heading] '>{t("Keyword Search")}</p>
						...
						<div className='flex-1 flex flex-col w-full md:w-fit gap-y-4 md:mr-3'>
							{filterData.map((el, i) => (
								<a key={i + 1} href="/#">
									<Card className='text-sm shadow-sm hover:shadow-lg hover:scale-[1.005] transition-transform duration-75'>
										<CardHeader className='flex items-start justify-between flex-row '>
											...
											<img src="/images/companyLogo_placeholder.svg" alt="Company Logo" className='block size-8 lg:size-12 md:size-10 ' />
										</CardHeader>
										<CardContent className='text-[11px] md:text-xs flex flex-col md:flex-row md:items-center  md:gap-x-4'>
											...
											<p className='flex items-center gap-x-1 text-[--primary-text]'> <Calendar className='size-4 text-inherit' /> {t(el.Experience)}  </p>
											<p className='flex items-center gap-x-1 text-[--primary-text]'> <Banknote className='size-4 text-inherit' /> ₹ {t(el.CTC)}/month </p>
										</CardContent>
										<CardFooter className="flex items-center  justify-between">
											...
											<Link to={`/detailjob?q=${el._id}`}>
												<button className='bg-transparent text-[--text-color-dark] border border-[--button-background-color-border] hover:bg-[--button-background-color-border] hover:text-[#fff] px-2 py-1 lg:px-4 lg:py-2'>{t("View In Deatils")}</button></Link>
										</CardFooter>
									</Card>
								</a>
							))}
