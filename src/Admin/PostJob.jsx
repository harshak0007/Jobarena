...
function PostJOb() {
  ...
  const sendData = (e) => {
    ...
    if (title === '' &&
      ...
      axios.post("https://jobarena-backend.onrender.com/api/job", bodyJosn).then((res) => {
  ...
  return (
    <div className=" py-6 sm:py-8 lg:py-12">
      <div className="mx-auto max-w-screen-2xl px-4 md:px-8 text-[--text-color-dark]">
        <div className="mb-10 md:mb-16">
          <h2 className="mb-4 text-center text-2xl font-bold  md:mb-6 lg:text-3xl">{t("Post a Job")}</h2>
        ...
        <form className="mx-auto grid max-w-screen-md gap-4 sm:grid-cols-2" onSubmit={sendData}>
          ...
          <div className="sm:col-span-2">
            <label htmlFor="aboutInternship" className="mb-2 inline-block text-sm  sm:text-base">{t("AboutJob")}*</label>
            <textarea name="aboutInternship" value={aboutInternship} onChange={(e) => setAboutInternship(e.target.value)} className="h-32 md:h-64 w-full rounded border bg-gray-50 px-3 py-2  outline-none ring-indigo-300 transition duration-100 focus:ring"></textarea>
          </div>
          <div className="sm:col-span-2">
          ...
          <div className="sm:col-span-2">
            <label htmlFor="CTC" className="mb-2 inline-block text-sm  sm:text-base">{t("CTC")}*</label>
          ...
          <button className='bg-[--button-background-color-border] w-[50%] mx-auto md:mx-0 text-[#fff] border  hover:text-[--text-color-dark] hover:border-[--button-background-color-hover] text-sm md:text-base hover:bg-transparent rounded-full px-1 py-1 lg:px-6 lg:py-2'>{t("Post Job")}</button>
        </form>
      </div>
    </div>
  )
